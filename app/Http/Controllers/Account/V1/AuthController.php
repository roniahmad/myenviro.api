...
class AuthController extends BaseApiController
{
    ...
    public function login(Request $request)
    {
        ...
        mp.nip, mp.nama, mp.gelar_depan, mp.gelar_belakang, mp.jabatan, mrf.deskripsi,
        mr.kode, mr.nama
        from aplikasi.users u
        left join master.user_organisasi uo on (u.email=uo.email)
        left join master.pegawai mp on (uo.pegawai_id=mp.id)
        left join master.perusahaan mr on (mr.id=uo.organisasi_id)
        left join master.referensi mrf on (mp.jabatan=mrf.id)
        where u.email ='pep@myenviro.id'
        and mrf.jenis =18
        */
        ...
        $res = User::leftJoin('master.user_organisasi as uo', function($joinU){
                    ...
                    ->leftJoin('master.perusahaan as mr', function($joinMR){
                        $joinMR->On('mr.id','=','uo.organisasi_id');
                    })
                    ->leftJoin('master.referensi as mrf', function($joinMRF){
                        $joinMRF->On('mrf.id','=','mp.jabatan');
                    })
                    ->select(
                        'users.verified','users.email',
                        'uo.tipe_user', 'uo.organisasi_id','uo.pegawai_id',
                        'mp.nip', 'mp.nama', 'mp.gelar_depan', 'mp.gelar_belakang', 'mp.jabatan', 'mrf.deskripsi',
                        'mr.kode', DB::raw('mr.nama as perusahaan')
                    )
                    ->where('users.email',$request->email)
                    ->where('mrf.jenis', $jenis_jabatan)
                    ->first();
