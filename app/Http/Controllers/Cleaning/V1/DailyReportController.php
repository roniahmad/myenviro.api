...
class DailyReportController extends BaseApiController
{
    ...
    */
    ...
    public function addDailyReportRecommendation(Request $request)
    {
        $validator = Validator::make($request->all(),[
            'dac_id' => 'required|integer',
            'jos_id' => 'required|integer',
        ...
        $id = $request->dac_id;
        $josid = $request->jos_id;
        $recom = $request->recommendation;

        $today = Carbon::now()->format('Y-m-d H:i:s');

        $avDaily = DailyReport::where('id', $id)->where('jos_id', $josid)->update(['rekomendasi'=>$recom, 'date_rekomendasi'=>$today]);

        if($avDaily>0){

            return $this->respond([
                'success' => 1,
                'message'=> Config('constants.messages.daily_report_recommendation_added_ok')]
            );
            ...
            return $this->respond([
                'success' => 0,
                'message'=> Config('constants.messages.daily_report_recommendation_added_fail')]
            );
